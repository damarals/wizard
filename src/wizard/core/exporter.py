...
def export_articles(articles: List[Article], filename: str, fields: Optional[List[str]] = None) -> str:
    """
    ...
    try:
        ...
        if fields is None:
            ...
            all_fields = {
                'search_term', 'title', 'authors', 'publication_date', 'journal',
                'volume', 'issue', 'pages', 'doi', 'abstract', 'topics',
                'article_id', 'issn', 'language', 'publisher',
                'is_open_access', 'is_peer_reviewed', 'citation_count', 'reader_count',
                'detail_url'
            }
