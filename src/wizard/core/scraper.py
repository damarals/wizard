...
class ArticleScraper:
    ...
    def _extract_doi(self, url: str) -> Optional[str]:
        ...
        """
        doi_pattern = re.compile(r"(?:doi\.org\/|doi=|\/doi\/)(10\.\d{4,9}\/[-._;()/:A-Z0-9]+)", re.IGNORECASE)
    ...
    def _get_total_pages(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            if soup.select('#resultados .result-busca'):
                ...
                logger.warning("Could not determine total pages, but found results. Assuming at least 1 page.")
                return 1
    ...
    def scrape_article_detail(self, article_id: str) -> dict:
        ...
        try:
            ...
            # Extract publisher
            publisher_elem = soup.select_one('b:contains("Editora")') or soup.select_one('b:contains("Publisher")')
            if publisher_elem and publisher_elem.parent:
                metadata['publisher'] = publisher_elem.parent.text.replace('Editora:', '').replace('Publisher:', '').strip()
            ...
            citation_elem = soup.select_one('.ppp-count:contains("Citation Indexes")')
            if citation_elem:
            ...
            reader_elem = soup.select_one('.ppp-count:contains("Readers")')
            if reader_elem:
            ...
            pub_info_elem = soup.select_one('.text-down-01:contains(")")')
            if pub_info_elem:
    ...
    def get_all_article_listings(self, search_dict: Dict[str, str], advanced: bool = True, 
                                callback: Callable[[int, str], None] = None) -> Dict[str, List[Dict]]:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for theme, query in search_dict.items():
                future = executor.submit(self._get_listings_for_term, query, theme, advanced, callback)
            ...
            for future in concurrent.futures.as_completed(future_to_theme):
                ...
                try:
                    ...
                    if callback:
                        ...
                        callback(progress, f"Collected {len(listings)} article listings for theme '{theme}'")
    ...
    def _get_listings_for_term(self, search_term: str, theme: str, is_advanced: bool, 
                               callback: Callable[[int, str], None] = None) -> List[Dict]:
        ...
        try:
            ...
            for page_num in range(2, total_pages + 1):
                ...
                page_url = self._construct_search_url(search_term, advanced=is_advanced, page=page_num)
                ...
                try:
                    ...
                    logger.error(f"Unexpected error processing page {page_num} for theme '{theme}': {e}")
                    continue
    ...
    def _extract_basic_article_info(self, soup: BeautifulSoup, theme: str, search_term: str) -> List[Dict]:
    ...
    def fetch_article_details(self, article_listings: Dict[str, List[Dict]], 
                              callback: Callable[[int, str], None] = None) -> List[Article]:
        """
        ...
        logger.info(f"Completed fetching details for {len(articles)} articles (out of {total_articles})")
        return articles

    def search(self, search_dict: Dict[str, str], advanced: bool = True, 
               callback: Callable[[int, str], None] = None) -> List[Article]:
        """
        ...
        logger.info(f"PHASE 1 COMPLETE: Found {total_listings} articles across {len(article_listings)} themes")
